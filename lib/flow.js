...
let profile = function profile(cb) {
    ...
    if (process.argv.slice(2).indexOf('-I') >= 0) {
        singleLocaleMode = true;
    }
    ...
    for (let testSuiteName in allTestCases) {
        allTestCases[testSuiteName].testcases.forEach(function (testCaseName) {
            ...
            if (locales.length > 0) {
                ...
                allBases[testCaseName].data = data.getTestsData(testCaseName, dataProviderFile, urls, singleLocaleMode);
            }
    ...
    //Parent Edit: var profiles = this.program.profile;
    //Parent Edit: profiles = profiles || 'base';
